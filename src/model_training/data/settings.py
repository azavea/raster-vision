...
class PotsdamInfo():
    def __init__(self):
        ...
        self.file_names = list(map(file_inds_to_name, self.file_inds))
        ...
        self.test_file_names = list(map(file_inds_to_name, self.test_file_inds))

        self.setup()

    def setup_file_names(self):
        nb_train_inds = int(round(self.train_ratio * len(self.file_inds)))
        train_inds = self.file_inds[0:nb_train_inds]
        validation_inds = self.file_inds[nb_train_inds:]
    ...
    def setup_channels(self):
        ...
        self.input_shape = (self.tile_size, self.tile_size,
                            self.nb_channels)
