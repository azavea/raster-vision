...
class TFObjectDetectionConfig(BackendConfig):
    ...
    def to_proto(self):
        ...
        conf = json_format.ParseDict(d, BackendConfigMsg.TFObjectDetectionConfig())
...
class TFObjectDetectionConfigBuilder(BackendConfigBuilder):
    def __init__(self, prev=None):
        config = { }
        if prev:
            config = { "tfod_config": prev.tfod_config,
    ...
    def from_proto(self, msg):
        ...
        b = b.with_script_locations(train_uri=conf.train_py,
    ...
    def build(self):
        ...
        if pretrained_model:
            b = b.with_config({
                "fineTuneCheckpoint": pretrained_model
            })

        for config_mod, ignore_missing_keys in b.config_mods:
            set_nested_keys(b.config['tfod_config'], config_mod, ignore_missing_keys)
    ...
    def with_train_options(self,
        ...
        """
        b = deepcopy(self)
        b.config['train_options'] = TFObjectDetectionConfig.TrainOptions(sync_interval,
                                                                         do_monitoring)
        return b
    ...
    def with_script_locations(self,
                         ...
                         eval_uri=DEFAULT_SCRIPT_EVAL,
                         export_uri=DEFAULT_SCRIPT_EXPORT):
        l = TFObjectDetectionConfig.ScriptLocations(train_uri,
                                                    eval_uri,
                                                    export_uri)
        b = deepcopy(self)
