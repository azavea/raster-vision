...
def png_to_geojson(geotiff_path, annotation_png_path, output_path,
                   object_half_len):
    """Convert COWC PNG annotations to GeoJSON format.
    ...
    annotation_im = np.array(Image.open(annotation_png_path))

    point_inds = np.argwhere(annotation_im[:, :, 0] != 0).astype(np.float)

    # Normalize inds
    point_inds[:, 0] /= annotation_im.shape[0]
    point_inds[:, 1] /= annotation_im.shape[1]
...
@click.argument('annotation_png_dir')
@click.argument('output_dir')
@click.option('--object-half-len', default=50)
def prepare_potsdam(geotiff_dir, annotation_png_dir, output_dir,
                    object_half_len):
    annotation_paths = glob.glob(os.path.join(
        annotation_png_dir, 'top_potsdam_*_RGB_Annotated_Cars.png'))
    make_empty_dir(output_dir)

    for annotation_path in annotation_paths:
        geotiff_base = os.path.basename(annotation_path)[0:-19]
        geotiff_path = os.path.join(geotiff_dir, geotiff_base + 'IR.tif')
        output_path = os.path.join(output_dir, geotiff_base + 'IR.json')
        boxlist = png_to_geojson(geotiff_path, annotation_path, output_path,
