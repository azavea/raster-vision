...
A pipeline is a sequence of commands that can be run locally or remotely. The ``rastervision2.core`` package provides pipelines for doing chip classification, object detection, and semantic segmentation on geospatial imagery. Pipelines are configured programmatically using a compositional API based on the `Pydantic library <https://pydantic-docs.helpmanual.io/>`_.

In the ``tiny_spacenet.py`` example from the :ref:`quickstart`, the ``get_config`` function returns a ``SemanticSegmentationConfig`` that Raster Vision finds when executing ``rastervision2 run tiny_spacenet.py``. It's also possible to provide a ``get_configs`` function which returns a list of ``PipelineConfig`` objects which can be run in parallel.
...
``rastervision2.core.ChipClassificationConfig``
...
``rastervision2.core.ObjectDetectionConfig``
...
``rastervision2.core.SemanticSegmentationConfig``
